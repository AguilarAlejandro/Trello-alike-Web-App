...
boardCtrl.deleteBoard = async (req, res) => {
  const currentBoard = await board.findById(req.params.id);
  if (currentBoard.createdBy != req.user.id) {
    // Unnecesary because this works on a POST method
...
boardCtrl.renderlists = async (req, res) => {
  ...
  const currentlists = await list
    .find({ boardId: req.params.id })
    .sort({ position: 1 });
  const currentCards = await card
    .find({ boardId: req.params.id })
    .sort({ position: 1 });
...
boardCtrl.updatelist = async (req, res) => {
  ...
  await list.findByIdAndUpdate(currentlistId, {
    listTitle: title,
...
boardCtrl.deletelist = async (req, res) => {
  ...
  if (currentlist.addedBy != req.user.id) {
    // Unnecesary because this works on a POST method
