            ...
            function (token, user, done) {
                ...
                transporter.sendMail(mailOptions, function (error, info) {
                    if (error) {
                        ...
                        req.flash(
                            "success",
                            "An email with further information has been sent to you"
                        );
                        res.redirect("/");
                    }
                });
...
userCtrl.processPasswordReset = async (req, res) => {
    ...
    user.findOne({ resetPasswordToken: req.params.token }, function (
        err,
        user
    ) {
        if (!user) {
    ...
    if (!currentUser) {
        ...
        if (password != confirmPassword) {
            ...
            if (password.length < 5) {
                ...
                currentUser.password = await currentUser.encryptPassword(
                    password
                );
